...
export default function Dashboard() {
  ...
  async function handleCancel(id) {
    ...
    const updated = await getReservationsForUser(user.id);
            ...
            {upcoming.map((r) => (
              <div key={r.id} className="card dash__row">
                <div>
                  ...
                  <h3>{r.tripTitle}</h3>
                  <p>
                    {formatDate(r.departure)} · {r.travelers} traveler
                    {r.travelers > 1 ? "s" : ""} · ${r.total}
                  </p>
            ...
            <div className="dash__list">
              {cancelled.map((r) => (
                <div key={r.id} className="card dash__row dash__row--cancelled">
                  <div>
                    <span className="badge dash__badge-cancelled">
                      Cancelled
                    </span>
                    <h3>{r.tripTitle}</h3>
                    <p>
